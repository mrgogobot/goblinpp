...
use crate::evaluator::MAX_FUNCTION_DEPTH;
...
fn reject_uncompilable_data_calls(program: &Program) -> Result<()> {
    ...
    fn visit_stmt(statement: &Stmt) -> bool {
        match statement {
            Stmt::Function { body, .. } => body.iter().any(visit_stmt),
            Stmt::Return(expr) => visit(expr),
...
fn generate(program: &Program, blocks: &[InlineRustBlock]) -> Result<String> {
    ...
    functions.push_str(&format!(
        "    if goblin_call_depth > {MAX_FUNCTION_DEPTH} {{ return Err(\"g_func call depth exceeded {MAX_FUNCTION_DEPTH}.\".into()); }}\n    match name {{\n"
    ));
    for statement in &program.statements {
        if let Stmt::Function { name, params, body } = statement {
            functions.push_str(&format!("        {name:?} => {{\n"));
            functions.push_str(&format!("            if args.len() != {} {{ return Err(format!(\"g_func {name} expects {} argument(s), got {{}}.\", args.len())); }}\n", params.len(), params.len()));
            functions.push_str("            let mut goblin_env: HashMap<String, Value> = HashMap::new();\n            let mut args = args.into_iter();\n");
            for param in params {
                functions.push_str(&format!(
                    "            goblin_env.insert({param:?}.into(), args.next().unwrap());\n"
                ));
            }
            functions.push_str(&generate_statements(body, blocks)?);
            functions.push_str(&format!("            Err(\"g_func {name} reached the end without return.\".into())\n        }}\n"));
        }
    }
    functions.push_str("        _ => Err(unknown(name)),\n    }\n}\n");
...
fn binary(op: char, left: Value, right: Value) -> Result<Value, String> {{
    if op == '+' {{
        if let (Value::Text(a), Value::Text(b)) = (&left, &right) {{
            return goblin_text::concat(a, b).map(Value::Text);
        }}
    }}
...
fn array_len(array: Value) -> Result<Value, String> {{
    let length = match array {{ Value::Array(items) => items.len(), Value::Text(text) => goblin_text::length(&text), _ => return Err("len() requires an array or text.".into()) }};
    Value::scalar(length as f64)
}}
...
fn generate_expr(expression: &Expr) -> Result<String> {
    Ok(match expression {
        ...
        Expr::Call { name, args } if name == "len" => {
            if args.len() != 1 {
                return Err(GoblinError::compile(
                    "len() requires one array or text value.",
                ));
            }

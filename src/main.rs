...
fn command_capabilities(json_output: bool) -> Result<i32> {
    let report = json!({
        ...
        "available": ["rust interpreter", "optional GO_PARANOID postflight evidence and self-verification", "optional seal value snapshots", "bounded for/while loops, if/else-if/else, switch/case/default, and Boolean comparisons", "g_func user-defined functions with local copy-value arguments and explicit return in both engines", "interactive input and argc/argv program arguments with hashed evidence", "text concatenation, Unicode-scalar length, checked numeric conversion and g_strings built-ins in both engines", "copy-value arrays with indexing, half-open slices, append and len in both engines", "native scalar and array compiler", "exact-hash inline Rust authorization", "native streaming FITS multi-HDU discovery", "native FITS image access", "native FITS binary-table scalar access and numeric statistics", "checksum-addressed deduplicated FITS evidence", "audited TXT, Markdown, CSV, TSV, and JSON output in either mode", "deterministic SVG and PNG FITS plots", "run receipts", "freeze enforcement", "revision lineage", "run verification", "semantic diff", "checksum custody ledger"],
        "string_functions": ["to_text", "parse_number", "str_trim", "str_contains", "str_replace", "str_split", "str_join", "len"],

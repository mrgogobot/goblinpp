...
The Goblin++ folder is already a local source-repository draft. Nothing in it
is on GitHub merely because it has a `.git` directory. Think of the terms this
way: a **repository** is the project folder plus its change history; a
...
1. Review [the release checklist](../RELEASE_CHECKLIST.md). In particular,
   choose a license before making the project public. This source draft has no
   license yet, and the editor extension currently says `UNLICENSED`.
...
After licensing, review, and passing checks, decide whether to make the
